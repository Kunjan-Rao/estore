...
const addForgotPasswordEmailTemplates_en = async (db) => {
	const emailTemplatesCount = await db.collection('emailTemplates').countDocuments({ name: 'forgot_password_en' });
	const emailTemplatesNotExists = emailTemplatesCount === 0;
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
...
const addForgotPasswordEmailTemplates_ru = async (db) => {
	const emailTemplatesCount = await db.collection('emailTemplates').countDocuments({ name: 'forgot_password_ru' });
	const emailTemplatesNotExists = emailTemplatesCount === 0;
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
...
const addForgotPasswordEmailTemplates_de = async (db) => {
	const emailTemplatesCount = await db.collection('emailTemplates').countDocuments({ name: 'forgot_password_de' });
	const emailTemplatesNotExists = emailTemplatesCount === 0;
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
...
const addRegisterDoiEmailTemplates_en = async (db) => {
	const emailTemplatesCount = await db.collection('emailTemplates').countDocuments({ name: 'register_doi_en' });
	const emailTemplatesNotExists = emailTemplatesCount === 0;
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
...
const addRegisterDoiEmailTemplates_ru = async (db) => {
	const emailTemplatesCount = await db.collection('emailTemplates').countDocuments({ name: 'register_doi_ru' });
	const emailTemplatesNotExists = emailTemplatesCount === 0;
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
...
const addRegisterDoiEmailTemplates_de = async (db) => {
	const emailTemplatesCount = await db.collection('emailTemplates').countDocuments({ name: 'register_doi_de' });
	const emailTemplatesNotExists = emailTemplatesCount === 0;
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
...
const createIndex = (db, collectionName, fields, options) => db.collection(collectionName).createIndex(fields, options);
...
const createAllIndexes = async (db) => {
	...
	const productCategoriesIndexes = await db.collection('productCategories').listIndexes().toArray();
	...
	if (productsIndexes.length === 1) {
		...
		await createIndex(db, 'products', { code: 1 });
	...
	const productItemsIndexes = await db.collection('productItems').listIndexes().toArray();

	if (productItemsIndexes.length === 1) {
		await createIndex(db, 'productItems', { slug: 1 });
		// await createIndex(db, 'productItems', { enabled: 1 });
		await createIndex(db, 'productItems', { sku: 1 });
		await createIndex(
			db,
			'productItems',
			{
		...
		winston.info('- Created indexes for: productItems');
	}

	const customersIndexes = await db.collection('customers').listIndexes().toArray();
...
(async () => {
	...
	await db.createCollection('productItems');
	await db.createCollection('TLorders');
	await db.createCollection('customers');
